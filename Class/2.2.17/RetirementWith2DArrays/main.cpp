/* 
  ...
  Created on February 2, 2017, 11:00 AM
  Purpose:  Retirement Calculator with 2D Arrays
 */
...
int main(int argc, char** argv) {    
    ...
    const int SIZE=144;
    int nYears=SIZE/2+1;
    float balance[SIZE][COLS]={};//Initialize 2D Array
...
void  prntAry(float balance[][COLS], int n){
    ...
    for(int i=0;i<n;i++){
        cout<<setw(3)<<static_cast<int>(balance[i][0])<<"   $"<<setw(11)<<fixed
                <<setprecision(2)<<balance[i][1]<<endl;
    }
    cout<<endl;
}
