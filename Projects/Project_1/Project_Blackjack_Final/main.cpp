...
int main(int argc, char** argv) {
    ...
    do{
        ...
        cin>>kPlay;
        while (kPlay!='Y'&&kPlay!='y'&&kPlay!='N'&&kPlay!='n'){
            cout<<"Please enter Y or N: ";
            cin>>kPlay;
        }
    }while (kPlay=='Y'||kPlay=='y');
    //Export stats to file
    cout<<"Thanks for playing, your stats are available at blackstats.txt!"<<endl;
    out<<name;
    out<<endl;
    out<<"Total Games: "<<tGames<<endl;
    out<<endl;
    out<<"Wins:        "<<setw(5)<<right<<wins<<endl;
    out<<endl;
    out<<"Losses:      "<<setw(5)<<right<<lose<<endl;
    out<<endl;
    out<<"Pushes:      "<<setw(5)<<right<<push<<endl;
    out<<endl;
    out<<"Win percentage: "<<setw(7)<<right<<fixed<<setprecision(2)<<(static_cast<float>(wins)/tGames)*PERCENT<<"%"<<endl;
    out<<endl;
    out<<"Loss percentage: "<<setw(7)<<right<<fixed<<setprecision(2)<<(static_cast<float>(lose)/tGames)*PERCENT<<"%"<<endl;
    out<<endl;
    out<<"Push percentage: "<<setw(7)<<right<<fixed<<setprecision(2)<<(static_cast<float>(push)/tGames)*PERCENT<<"%"<<endl;
